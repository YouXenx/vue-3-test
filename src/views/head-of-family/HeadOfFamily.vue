...
const fetchData = async () => {
  try {
    ...
    headOfFamily.value = response?.data || response;
  } catch (error) {
      ...
      <div class="flex flex-col w-[calc(525/1000*100%)] shrink-0 gap-[14px]">
        ...
        >
          ...
          <div class="flex items-center gap-4">
            ...
            >
              <div v-if="headOfFamily?.user">
                <img
                  :src="headOfFamily?.profile_picture"
                  @error="$event.target.src = 'https://picsum.photos/640/480'"
                />
              </div>
            </div>
          ...
          <div class="flex items-center justify-between">
            <div class="flex flex-col gap-[6px]">
              <p class="font-semibold text-[32px] leading-10">
                {{ headOfFamily?.family_members?.length || 0 }}
              </p>
                ...
                <div class="flex flex-col gap-[6px] w-full">
                  ...
                  <p class="flex items-center gap-1">
                    <img
                      ...
                      @error="$event.target.src = 'https://picsum.photos/18/18'"
                      class="flex size-[18px] shrink-0"
                      alt="icon"
                    />
                    <span class="font-medium text-sm text-desa-secondary">{{
      ...
      >
        ...
        <div class="flex flex-col p-4 gap-3">
          ...
          <div class="flex items-center gap-3">
            ...
            <button
              class="flex items-center justify-center h-14 rounded-2xl py-3 px-8 gap-[10px] bg-desa-red w-full disabled:opacity-50 disabled:cursor-not-allowed"
              @click="handleDelete"
              :disabled="loading"
            >
              <span v-if="!loading" class="flex items-center gap-[10px]">
                <img
                  src="@/assets/images/icons/trash-white.svg"
                  class="flex size-6 shrink-0"
                  alt=""
